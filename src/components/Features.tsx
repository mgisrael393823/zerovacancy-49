...
export function FeaturesSectionWithHoverEffects() {
  return <section className="py-6 sm:py-8 px-4 sm:px-6 lg:px-8 bg-white">
...
const Feature = ({
  ...
  return <button className={cn("relative w-full text-left group", "rounded-xl transition-all duration-200", "bg-white/90 hover:bg-white", "border border-gray-200/80 hover:border-gray-300", "p-4 sm:p-5", "focus:outline-none focus:ring-2 focus:ring-primary/20", !isMobile && "hover:shadow-md hover:-translate-y-0.5")} onClick={handleClick} aria-expanded={isMobile ? isExpanded : undefined}>
      ...
      // Increased size for better visibility on desktop
      "rounded-xl",
      // More pronounced rounded corners
      "transition-all duration-200", colorScheme.bg, "group-hover:shadow-sm", "border border-indigo-200/30" // Subtle border for more dimension
      )}>
          <Icon className={cn("w-6 h-6",
        // Consistent icon size
        colorScheme.text, "transition-all duration-200", "group-hover:scale-110", isMobile && isExpanded && "transform rotate-90")} />
        </div>
        
        <div className="text-left w-full">
          <h3 className={cn("text-base font-semibold leading-6 font-space mb-1.5", "text-gray-900 group-hover:text-indigo-600",
        // Matching hover color to icon theme
        "transition-colors duration-200")}>
            {title}
          </h3>
          <div className={cn("overflow-hidden transition-[max-height,opacity] duration-200", isMobile && !isExpanded ? "max-h-0 opacity-0" : "max-h-32 opacity-100")}>
